...
11. **Cookie strategy config**: `ttl` (duration), `forceVisible` (bool), `waitUntil` (`load`|`networkidle`|`domcontentloaded`|`commit`), `requiredCookies` (string[] — cookie names that must exist before auth is considered complete; use for sites where the entry page isn't a login page, e.g. QR code login), `cookiePaths` (string[] — additional URL paths to query when checking cookies; use for sites that set auth cookies on a sub-path like `/wiki` instead of `/`).
12. **Mode**: `mode: browser | browserless` config field (default `browser`). Set via `sig init --remote` on headless/remote machines. In `browserless` mode, `NullBrowserAdapter` is used and browser-dependent commands guide users to `sig sync pull`, `--cookie`, or `--token`.
13. **localStorage**: Provider-level `localStorage` config in `config.yaml` (`LocalStorageConfig[]`). Each entry has `name` (output key), `key` (localStorage key to read), and optional `jsonPath` (dot-delimited path into parsed JSON value). Extracted after browser auth, stored on the credential as `localStorage: Record<string, string>`, included in `sig get` JSON output but NOT applied as HTTP headers. Useful for Slack (xoxc token in localStorage alongside xoxd cookie).
