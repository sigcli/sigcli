...
12. **Cookie strategy config**: `ttl` (duration), `forceVisible` (bool), `waitUntil` (`load`|`networkidle`|`domcontentloaded`|`commit`), `requiredCookies` (string[] — cookie names that must exist before auth is considered complete; use for sites where the entry page isn't a login page, e.g. QR code login), `cookiePaths` (string[] — additional URL paths to query when checking cookies; use for sites that set auth cookies on a sub-path like `/wiki` instead of `/`).
